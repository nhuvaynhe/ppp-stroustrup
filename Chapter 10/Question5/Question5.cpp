...
// if s is the name of month? if so return its index [0:11] otherwise -1
{
    for (int i=0; i<12; ++i){
...
// format: {month feb...}
{
    ...
    if (duplicates) error("deplicate readings in month", duplicates);
...
//-------------------------------------------------------------------------- 
void print_day(ofstream& ofs, const Day& d,int index)
{   
    string space = "    ";
    for (int i=0; i<24; ++i)
    {
...
void print_year(ofstream& ofs, const Year& y)
{
    ...
    for (Month m : y.month)
    {
        if (m.month != not_a_month) print_month(ofs, m);
    } 
}
//--------------------------------------------------------------------------
int main()
