...
void set_value(string s, double d){						// Put a new value into name
	for (int i = 0; i<names.size(); ++i) {
        if (names[i].const_name == s) error("can't change value of const var");
...
double setconst(){      //Set a constvalue
	...
	double d = expression();
	names.push_back(Variable(name,name,d));
	return d;
}
//------------------------------------------------------------------------------
double declaration(){
...
int main(){
	try {
		//predefine names:
    	names.push_back(Variable("pi","pi", 3.14159));
    	names.push_back(Variable("e", "e", 2.718281828));
    	names.push_back(Variable("k", "k", 1000));
